...
def match_targets(table):
    ...
    nearest_matches, nearest_separations, nearest_distances = match_coordinates_sky(table['coords'], target_coords['coords'])
    mask = nearest_separations > (1e-3 * u.deg)
    table['matched target'] = MaskedColumn(
        tuple(target_coords[n]['target_name'] for n in nearest_matches),
        mask=mask,
    )
    table['matched target separation'] = MaskedColumn(
        nearest_separations,
        mask=mask,
    )