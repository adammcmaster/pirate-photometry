...
def get_refstar_table(
    target_row, obs_meta, obs_table, calibration_catalogue, calibration_catalogue_fields
):
    ...
    refstar_table_path = (
        f"data/calibration_tables/{target_row['matched target']}/"
        f"{calibration_catalogue.replace('/', '_')}_{obs_meta['band']}.ecsv"
    )
    try:
        ...
        obs_table["flux_diff"] = numpy.abs(
            obs_table["FLUX_AUTO"] - target_row["FLUX_AUTO"]
        )
        ...
        for i in range(total_iterations):
            ...
            catalogue_results = Vizier.query_region(
                sources, radius=1e-4 * u.deg, catalog=calibration_catalogue
            )
            ...
            catalogue_matches = catalogue_matches[
                ~numpy.isnan(
                    catalogue_matches[calibration_catalogue_fields[obs_meta["band"]]]
                )
            ]
            ...
            for catalogue_row in catalogue_matches:
                refstar_table["coords"].append(
                    sources[int(catalogue_row["_q"]) - 1]["coords"]
                )
                refstar_table["mag"].append(
                    tuple(
        ...
        if len(refstar_table) < MIN_CALIBRATION_ROWS:
            return None
        refstar_table.write(
            refstar_table_path, overwrite=True,
        )
...
def match_refstars(obs_table, refstar_table):
    ...
    calibration_table["ref mag"] = tuple(
        refstar_table[n]["mag"] for n in nearest_matches
    )
...
def calibrate(target_row, obs_meta, obs_table):
    ...
    Path(f"data/calibration_tables/{target_row['matched target']}").mkdir(
        parents=True, exist_ok=True
    )
    for (
        calibration_catalogue,
        calibration_catalogue_fields,
    ) in CALIBRATION_CATALOGUES.items():
        if obs_meta["band"] not in calibration_catalogue_fields:
            continue
        refstar_table = get_refstar_table(
            target_row,
            obs_meta,
            obs_table,
            calibration_catalogue,
        ...
        calibrated_mags = calibration_table["ref mag"] - 2.5 * numpy.log10(
            target_row["FLUX_AUTO"] / calibration_table["FLUX_AUTO"]
        )
        ...
        out_table.add_columns(
            [
                ...
                # Standard error as per
                # https://www.statology.org/standard-error-of-mean-python/
                numpy.std(calibrated_mags, ddof=1)
                / numpy.sqrt(numpy.size(calibrated_mags)),
                calibration_catalogue,
            ],
